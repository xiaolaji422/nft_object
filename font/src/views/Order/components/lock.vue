    ...
    <div :class="$style.warpper">
        <div :class="$style.form">
            <div :class="$style['item-ctn']">
                ...
                <el-select style="width: calc(100% - 120px);" v-model="formData.album_name" filterable remote
                    reserve-keyword placeholder="请输入商品搜索" :remote-method="remoteMethod" :loading="loading"
                    value-key="id" @change="changeGoods">
                    <el-option v-for="item in goodsData" :key="item.id" :label="item.name" :value="item" />
                </el-select>
            </div>
                ...
                <el-form>
                    <el-form-item label="账号">
                        <el-select v-model="formData.account_id" class="m-2" placeholder="选择账号">
                            <el-option v-for="item in account" :key="item.id" :label="item.account" :value="item.id" />
                        </el-select>
                    </el-form-item>
                    ...
                    <el-row :gutter="40">
                        <el-col :span="8" v-if="!formData.id">
                            <el-button size="small" @click="lockGoods" type="primary">自动锁单</el-button>
                        </el-col>
                        <el-col :span="8" v-else>
                            <el-button size="small" @click="unlockGoods" type="primary">取消锁单</el-button>
                        </el-col>
                    </el-row>
...
const unlockGoods = async () => {
    const res = await lockApi.Cancel({ id: formData.value.id })
    console.log(res, "lockGoods", formData.value)
    if (res) {
        notify.success("取消成功")
        emit("refresh")
